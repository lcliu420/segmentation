...
本目录是实验三中的基础 CNN baseline。代码直接读取 WL/NBI 胃镜病灶二分类分割数据，训练阶段只记录轻量指标，最终测试阶段保存逐图预测 mask，并输出逐图完整指标，用于后续和实验二的边界频率指标合并分析。
...
```text
../predictions/unet/WL/test/*.png
../predictions/unet/NBI/test/*.png
../outputs/prediction_metrics/unet_WL_prediction_metrics.csv
../outputs/prediction_metrics/unet_NBI_prediction_metrics.csv
```
...
python test.py --modal WL --data_root ../../dataset --output_dir ../outputs/runs/smoke_unet_WL --checkpoint ../outputs/runs/smoke_unet_WL/best_model.pth --num_workers 0 --limit_samples 2 --run_name smoke_unet_WL --model_name unet_smoke
```
