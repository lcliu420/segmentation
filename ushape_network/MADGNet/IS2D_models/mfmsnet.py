...
class MFMSNet(nn.Module):
    def __init__(self,
        ...
        self.feature_encoding = load_cnn_backbone_model(backbone_name=cnn_backbone, pretrained=pretrained_backbone)
    ...
    def _calculate_multi_task_criterion(self, outputs, region_gt, distance_gt, boundary_gt):
        total_loss = 0.0
        loss_dict = {"region": 0.0, "distance": 0.0, "boundary": 0.0}

        for map_pred, distance_pred, boundary_pred in outputs:
            region_loss = self.structure_loss(map_pred, region_gt)
            distance_loss = F.mse_loss(torch.sigmoid(distance_pred), distance_gt)
            boundary_loss = F.binary_cross_entropy_with_logits(boundary_pred, boundary_gt)

            total_loss = total_loss + region_loss + distance_loss + boundary_loss
            loss_dict["region"] += region_loss.item()
            loss_dict["distance"] += distance_loss.item()
