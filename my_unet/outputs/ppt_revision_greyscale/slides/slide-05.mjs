...
export async function slide05(presentation, ctx) {
  ...
  base(slide, ctx, 5, "RESULTS", "outputs_new 验证集 best epoch 结果");
  const headers = ["模型", "epoch", "Dice", "Delta", "IoU", "B-IoU", "HD95", "MAE"];
  const widths = [116, 86, 132, 122, 132, 132, 132, 118];
  let x = 58;
  const y0 = 145;
  headers.forEach((h, i) => {
    box(slide, ctx, x, y0, widths[i], 40, { fill: C.dark, stroke: C.dark });
    text(slide, ctx, x + 6, y0 + 8, widths[i] - 12, 24, h, { size: 15, color: C.paper, bold: true, align: "center" });
    x += widths[i];
  });
  rows.forEach((r, ri) => {
    x = 58;
    ...
    r.forEach((v, ci) => {
      const fill = isBest ? C.dark : isBoundary ? C.soft : ri % 2 ? "#F9FAFB" : C.paper;
      const txt = isBest ? C.paper : C.ink;
      box(slide, ctx, x, y, widths[ci], 44, { fill, stroke: isBest ? C.dark : C.line, width: isBest ? 1.2 : 1 });
      text(slide, ctx, x + 5, y + 11, widths[ci] - 10, 22, v, {
        size: 13.5, color: txt, bold: isBest || (isBoundary && ci === 5), align: "center",
      });
      x += widths[ci];
    });
  });
