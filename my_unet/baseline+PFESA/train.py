...
from networks.pfesa_factory import available_pfesa_variants
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="../outputs/B+PFESA")
    ...
    parser.add_argument(
        ...
        type=str,
        default="x1",
        choices=["none", "x1", "x2", "x3", "x12", "x13", "x23", "x123"],
    )
    parser.add_argument("--pfesa_base_ratio", type=float, default=0.1)
    parser.add_argument("--pfesa_base_ratios", type=float, nargs=3, default=None, metavar=("X1", "X2", "X3"))
    parser.add_argument("--pfesa_variant", type=str, default="original", choices=available_pfesa_variants())
...
def resolve_pfesa_base_ratios(args):
    if args.pfesa_base_ratios is None:
        return [args.pfesa_base_ratio, args.pfesa_base_ratio, args.pfesa_base_ratio]
    if len(args.pfesa_base_ratios) != 3:
        raise ValueError("--pfesa_base_ratios must contain exactly 3 values for x1 x2 x3.")
    return [float(value) for value in args.pfesa_base_ratios]


def format_pfesa_base_ratios(args):
    ratios = resolve_pfesa_base_ratios(args)
    if args.pfesa_base_ratios is None:
        return format_param(args.pfesa_base_ratio)
    return "-".join(format_param(value) for value in ratios)
...
def build_run_name(args):
    ...
    return (
        ...
        f"_pfesa{args.pfesa_skip_mode}"
        f"_{args.pfesa_variant}"
        f"_r{format_pfesa_base_ratios(args)}"
    )
...
def save_best_result(run_dir, args, run_name, row, train_metrics, val_metrics, checkpoint_path):
    record = {
        ...
        "pfesa_base_ratios": resolve_pfesa_base_ratios(args),
        "args": vars(args).copy(),
    ...
    csv_row = {
        ...
        "pfesa_variant": args.pfesa_variant,
...
def main():
    ...
    model = CSwinUnet(
        ...
        pfesa_skip_mode=args.pfesa_skip_mode,
        pfesa_base_ratio=args.pfesa_base_ratio,
        pfesa_base_ratios=pfesa_base_ratios,
        pfesa_variant=args.pfesa_variant,
    ).to(device)
    ...
    config_record["pfesa_base_ratios_resolved"] = pfesa_base_ratios
