    ...
    class Rearrange(nn.Module):
        def __init__(self, pattern, **axes_lengths):
            ...
            if pattern != "b c h w -> b (h w) c":
                raise ValueError(f"Unsupported fallback Rearrange pattern: {pattern}")
...
from .pfesa_factory import build_token_pfesa
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=8, embed_dim=64, depth=[1, 2, 9, 1],
                 ...
                 pfesa_skip_mode="none", pfesa_base_ratio=0.1, pfesa_base_ratios=None,
                 pfesa_variant="original"):
        super().__init__()
        self.use_chk = use_chk
        valid_pfesa_modes = ("none", "x1", "x2", "x3", "x12", "x13", "x23", "x123")
        if pfesa_skip_mode not in valid_pfesa_modes:
            raise ValueError(f"Unsupported pfesa_skip_mode: {pfesa_skip_mode}")
        if pfesa_base_ratios is None:
            pfesa_base_ratios = (pfesa_base_ratio, pfesa_base_ratio, pfesa_base_ratio)
        if len(pfesa_base_ratios) != 3:
            raise ValueError(f"pfesa_base_ratios must contain 3 values for x1/x2/x3, got {pfesa_base_ratios}")
        x1_ratio, x2_ratio, x3_ratio = [float(value) for value in pfesa_base_ratios]
        self.pfesa_skip_mode = pfesa_skip_mode
        self.pfesa_variant = pfesa_variant
        self.pfesa_base_ratios = (x1_ratio, x2_ratio, x3_ratio)
        self.pfesa_skip_layers = set() if pfesa_skip_mode == "none" else set(pfesa_skip_mode[1:])
        self.pfesa_x1 = build_token_pfesa(variant=pfesa_variant, base_ratio=x1_ratio)
        self.pfesa_x2 = build_token_pfesa(variant=pfesa_variant, base_ratio=x2_ratio)
        self.pfesa_x3 = build_token_pfesa(variant=pfesa_variant, base_ratio=x3_ratio)
    ...
    def apply_pfesa_skip(self, x, skip_name):
        if skip_name[-1] in self.pfesa_skip_layers:
            return getattr(self, f"pfesa_{skip_name}")(x)
        return x
